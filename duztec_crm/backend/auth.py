...
def init() -> None:
    ...
    # users columns are migrated here (not in db.MIGRATIONS) because this is where the table is created
    for col, ddl in (("rkz", "TEXT DEFAULT ''"), ("password_hash", "TEXT DEFAULT ''"),
                     ...
                     ("last_seen", "TEXT DEFAULT ''")):   # browser heartbeat for Active / Idle / Out
        if col not in cols:
...
def list_users(request: Request):
    ...
    con = db.connect()
    out = db.rows(con.execute("SELECT id,email,name,role,active,rkz,created_at,last_login,last_seen,"
...
def login_history(request: Request, email: str = "", days: int = 30):
    ...
    sql = """SELECT at, action, detail FROM activity WHERE entity_type='user'
             AND action IN ('login','logout','session_expired','password_set','password_reset')
             AND at >= datetime('now', ?)"""
